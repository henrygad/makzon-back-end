...
export const sendVerificationOTP = async (
    ...
    try {        
        ...
        if (!req.session.user) return createError({ statusCode: 404, message: "User not found" });
        const { _id, userName, email, userVerified } = req.session.user;
        
        if (userVerified) createError({ statusCode: 400, message: "User is already verified" });
        ...
        const url =
            ...
            `/verify/user?email=${email}&otp=${otp}`;

        // Send OTP token to user mail box
        const result = await sendEmail({
            emailTo: email,
            subject: "Email Verification",
            template: `<span style="display: block;">Hi ${userName}</span> <br/> <span style="display: block;">Your verification code is: <span style="font-weight: bold;" >${otp}</span></span>, or click <a href="${url}">Verify account</a>`,
        });
        if (!result.sent)
            createError({
                statusCode: 500,
